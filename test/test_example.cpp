...
int main(int argc, char** argv) {
    ...
    std::string keyframe_url(argv[3]);
    std::string task_no(argv[4]);
    ...
    // 日志
    //设置日志
	// LogLevel logLevel = (LogLevel)LError;//
    ...
    auto pipeline = std::make_shared<pipeline::YoloDetectPipeline>(
        "test_pipeline", input_stream_url, output_stream_url, keyframe_url, task_no, trt_instance);
